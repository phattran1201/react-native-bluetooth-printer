...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(nullable NSError *)error{
    if (error){
        NSLog(@"Error scanning peripheral services: %@-> %@", peripheral.name, [error localizedDescription]);
        return;
    }
    NSLog(@"Discovered peripheral services: %@ -> %@",peripheral.name,peripheral.services);
    for (CBService *service in peripheral.services) {
        [peripheral discoverCharacteristics:nil forService:service];
         NSLog(@"Service ID: %@",service.UUID.UUIDString);
    }
    NSLog(@"Start scanning peripheral service characteristics %@...",peripheral.name);
