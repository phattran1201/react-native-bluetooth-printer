package rn.bluetooth.printer.tsc;
...
public class RNBluetoothTscPrinterModule extends ReactContextBaseJavaModule
  ...
  public void printLabel(final ReadableMap options, final Promise promise) {
    ...
    TscCommand.ENABLE enable = options.hasKey("tear")
        ? options.getString("tear").equalsIgnoreCase(TscCommand.ENABLE.ON.getValue()) ? TscCommand.ENABLE.ON
    ...
    TscCommand.DIRECTION direction = options.hasKey("direction")
        ? TscCommand.DIRECTION.BACKWARD.getValue() == options.getInt("direction")
            ? TscCommand.DIRECTION.BACKWARD
            : TscCommand.DIRECTION.FORWARD
        : TscCommand.DIRECTION.FORWARD;
    // Not Support Yet
    // TscCommand.MIRROR mirror = options.hasKey("mirror") ?
    // TscCommand.MIRROR.MIRROR.getValue() == options.getInt("mirror") ?
    ...
    if (speed != null) {
      tsc.addSpeed(speed);// Set print speed
    }
    if (density != null) {
      tsc.addDensity(density);// Set print density
    }
    tsc.addSize(width, height); // Set label size according to actual dimensions
    tsc.addGap(gap); // Set label gap according to actual size, set to 0 for gapless paper
    tsc.addDirection(direction);// Set print direction
    ...
    tsc.addTear(enable); // Enable tear mode
    if (home) {
      tsc.addBackFeed(16);
      tsc.addHome();// Feed paper to start position
    }
    ...
    for (int i = 0; texts != null && i < texts.size(); i++) {
      ...
      try {
        ...
        t = new String(temStr.getBytes("GB2312"), "GB2312");// Text to print
      } catch (Exception e) {
      ...
      tsc.addText(x, y, fonttype/* Font type */,
          rotation/* Rotation angle */, xscal/* Horizontal scale */, yscal/* Vertical scale */, t);

      if (bold) {
        tsc.addText(x + 1, y, fonttype,
            rotation, xscal, yscal, t/* May need to replace with spaces of equal length */);
