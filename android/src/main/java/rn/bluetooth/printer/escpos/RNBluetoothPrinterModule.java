...
package rn.bluetooth.printer.escpos;
...
import rn.bluetooth.printer.BluetoothService;
import rn.bluetooth.printer.BluetoothServiceStateObserver;
import rn.bluetooth.printer.escpos.command.sdk.Command;
import rn.bluetooth.printer.escpos.command.sdk.PrintPicture;
import rn.bluetooth.printer.escpos.command.sdk.PrinterCommand;

public class RNBluetoothPrinterModule extends ReactContextBaseJavaModule
    ...
    public RNBluetoothPrinterModule(ReactApplicationContext reactContext,
    ...
    public void printPic(String base64encodeStr, @Nullable ReadableMap options, final Promise promise) {
        try {
            if (base64encodeStr == null || base64encodeStr.isEmpty()) {
                promise.reject("INVALID_IMAGE", "Base64 image string is null or empty");
                return;
            }

            if (mService == null) {
                promise.reject("SERVICE_NOT_INITIALIZED", "Bluetooth service is not initialized");
                return;
            }

            if (mService.getState() != BluetoothService.STATE_CONNECTED) {
                promise.reject("NOT_CONNECTED", "Printer is not connected. Please connect to a printer first.");
                return;
            }
            ...
            if (bytes == null || bytes.length == 0) {
                promise.reject("INVALID_BASE64", "Failed to decode base64 image");
                return;
            }

            Bitmap mBitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.length);
            if (mBitmap == null) {
                promise.reject("INVALID_IMAGE_DATA",
                        "Failed to decode image. Make sure the base64 string is a valid image.");
                return;
            }
            ...
            promise.reject("INVALID_BASE64", "Invalid base64 format: " + e.getMessage());
        } catch (OutOfMemoryError e) {
            promise.reject("OUT_OF_MEMORY", "Image is too large to process. Try reducing image size.");
        } catch (Exception e) {
            promise.reject("PRINT_ERROR", "Failed to print image: " + e.getMessage());
        }
    }
    ...
    private boolean sendDataByte(byte[] data) {
        if (data == null) {
            Log.w(TAG, "sendDataByte: data is null");
            return false;
        }
        if (mService == null) {
            Log.e(TAG, "sendDataByte: Bluetooth service is not initialized");
            return false;
        }
        if (mService.getState() != BluetoothService.STATE_CONNECTED) {
            Log.w(TAG, "sendDataByte: Printer is not connected (state: " + mService.getState() + ")");
            return false;
        }
        try {
            mService.write(data);
            ...
            Log.e(TAG, "sendDataByte: Failed to write data: " + e.getMessage());
            return false;
        }
    }

    // Check if character is Chinese (CJK) based on Unicode block
