...
public class RNBluetoothManagerModule extends ReactContextBaseJavaModule
    ...
    public void connect(String address, final Promise promise) {
        try {
            if (address == null || address.isEmpty()) {
                promise.reject("INVALID_ADDRESS", "Bluetooth address is null or empty");
                return;
            }
            ...
            if (!adapter.isEnabled()) {
                promise.reject("BLUETOOTH_DISABLED", "Bluetooth is not enabled. Please enable Bluetooth first.");
                return;
            }
            ...
            if (!BluetoothAdapter.checkBluetoothAddress(address)) {
                promise.reject("INVALID_ADDRESS", "Invalid Bluetooth address format: " + address);
                return;
            }
            ...
            promise.reject("PERMISSION_DENIED", "Bluetooth permission denied. Please grant Bluetooth permissions.");
        } catch (IllegalArgumentException e) {
            promise.reject("INVALID_ADDRESS", "Invalid Bluetooth address: " + e.getMessage());
        } catch (Exception e) {
    ...
    public void isDeviceConnected(final Promise promise) {
        try {
            ...
            boolean isConnected = (state == BluetoothService.STATE_CONNECTED);
            promise.resolve(isConnected);
        } catch (Exception e) {
            Log.e(TAG, "isDeviceConnected error: " + e.getMessage());
            promise.resolve(false);
        }
    }
    ...
    public void getConnectedDeviceAddress(final Promise promise) {
        try {
            ...
            promise.resolve(address != null ? address : "");
        } catch (Exception e) {
            Log.e(TAG, "getConnectedDeviceAddress error: " + e.getMessage());
            promise.resolve("");
        }
    }
